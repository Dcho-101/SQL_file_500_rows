...
-- checking the table if properly executed or not
select * from user_details;


-- checking the number of user listed in this database

select distinct count(first_name) from user_details;

-- name will be in upper case

select first_name, upper(first_name) as upper_first_names
from user_details;

-- showing the first 100 user
SELECT * FROM user_details LIMIT 100;

-- starting the name of the list in upper case letter 
SELECT user_id,
CONCAT(UPPER(LEFT(first_name,1)),
LOWER(RIGHT(first_name,LENGTH(first_name)-1)))
AS name FROM user_details  
ORDER BY user_id ASC;

-- starting the name of gender in the list in upper case letter
SELECT user_id,
CONCAT(UPPER(LEFT(gender,1)),
LOWER(RIGHT(gender,length(gender)-1)))
AS gender FROM user_details
ORDER BY user_id ASC;
